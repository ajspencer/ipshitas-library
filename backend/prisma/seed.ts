...
// Initial book data from Ipshita's reading list
const initialBooks = [
  {
    id: '1',
...
async function main() {
  ...
  // Create reading goal
  console.log('Creating reading goal...');
  await prisma.readingGoal.create({
    data: initialReadingGoal,
  });

  // Create books with reviews
  console.log('Creating books with reviews...');
  for (const bookData of initialBooks) {
    const { reviews, ...book } = bookData;
  ...
  console.log(`✅ Seeded ${initialBooks.length} books`);
